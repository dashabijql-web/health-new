...
public class PressureService {
    ...
    public PressureService(PressureMapper pressureMapper, EmployeeMapper employeeMapper) {
        this.pressureMapper = pressureMapper;
        this.employeeMapper = employeeMapper;
    }

    public PressurePage list(String empCode, String startTime, String endTime, Integer page, Integer size) {
        ...
        long total = pressureMapper.countList(code, start, end);
        return new PressurePage(
                pressureMapper.findPage(code, start, end, offset, safeSize),
                total,
                safePage,
                safeSize);
    }

    public List<PressureRecord> trend(String empCode, String startTime, String endTime) {
        String code = requireEmpCode(empCode);
        return pressureMapper.findTrend(code, blankToNull(startTime), blankToNull(endTime));
    }

    public PressureRecord create(String empCode, Integer pressure, String recordTime) {
        String code = requireEmpCode(empCode);
        Employee employee = employeeMapper.findByEmpCode(code);
        if (employee == null) {
            throw new IllegalArgumentException("职工不存在");
        }
        if (pressure == null || pressure < 30 || pressure > 100) {
            throw new IllegalArgumentException("压力指数必须在30到100之间");
        }
        ...
        pressureMapper.insertRecord(tableName, code, pressure, time);
