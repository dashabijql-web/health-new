...
import { fetchWarningDetail, fetchWarningList, type WarningRecord } from '../api/warning'
...
async function openDetail(record: WarningRecord) {
  ...
  try {
    selected.value = await fetchWarningDetail(record.id, record.createTime)
  ...
  <section class="page-card warning-page">
    ...
    <article v-if="selected" class="warning-detail">
      <div class="warning-detail__header">
        ...
        <button type="button" class="button-secondary" @click="selected = null">关闭</button>
      </div>
      <dl>
        ...
        <div><dt>级别 / 状态</dt><dd>{{ selected.warningLevel }} / {{ selected.handled ? '已处理' : '待处理' }}</dd></div>
