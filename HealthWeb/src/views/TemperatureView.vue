...
const records = ref<TemperatureRecord[]>([])
const trend = ref<TemperatureRecord[]>([])
const newEmpCode = ref('')
const newTemperature = ref(36.7)
    ...
    series: [
      {
        type: 'line',
        name: '体温',
        data: trend.value.map((item) => item.temperature),
        smooth: true,
...
async function loadTemperatures() {
  ...
  message.value = '正在加载体温数据…'

  try {
    const [pageResult, trendResult] = await Promise.all([
      fetchTemperatureList(code, startTime.value, endTime.value, page.value, size),
      fetchTemperatureTrend(code, startTime.value, endTime.value),
    ])
...
async function submitCreate() {
  ...
  createMessage.value = '正在保存体温…'

  try {
    const created = await createTemperature(code, Number(newTemperature.value))
    createStatus.value = 'success'
    createMessage.value = `已写入 ${created.empName || created.empCode} 的体温 ${created.temperature.toFixed(1)}℃。`
    empCode.value = code
    page.value = 1
...
<template>
  <section class="page-card">
    <p class="eyebrow">阶段 8 · 体温</p>
    <h1>体温记录</h1>
    <p>读取现有 health 库中的体温字段，数据库按实际温度×10保存。必须先填工号，例如 EMP0001。图表最多展示 200 条。</p>
    ...
    <form v-if="canManageDepartments" class="dept-form" @submit.prevent="submitCreate">
      ...
      <label>
        <span>体温（℃）</span>
        <input v-model.number="newTemperature" type="number" min="35" max="42" step="0.1">
      </label>
    ...
    <form class="dept-form emp-form" @submit.prevent="searchTemperatures">
      <label>
        <span>工号</span>
