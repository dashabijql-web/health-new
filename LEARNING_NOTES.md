...
状态：已将处置审计记录作为事件状态变化时间线重新读取并展示；生命周期综合测试尚待下一步补齐。

### 时间线规则

- 每次确认、分派、处理、关闭或误报都会在同一事务内写入 `command_center_incident_action`，因此状态更新和审计记录要么一起成功，要么一起回滚。
- `GET /warning/{id}/timeline?occurredAt=yyyy-MM-dd HH:mm:ss` 使用预警 ID 与发生时间复合定位，先确认预警存在，再按操作时间正序读取审计记录。
- 时间线返回动作 ID、动作、结果、操作人、目标、备注和操作时间；无操作记录时返回空数组，不虚构初始事件。
- 预警详情同时读取详情、当前事件状态和时间线；动作成功后重新请求后端，展示数据库中的真实结果。

### 验证

- Service 测试覆盖预警存在性检查与时间线读取，Controller 测试覆盖接口响应字段。
- 前端类型检查与构建用于确认新增时间线类型、请求和页面渲染一致。
